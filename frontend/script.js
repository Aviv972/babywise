...
const API_ENDPOINTS = {
    ...
    RESET: '/api/reset',
    CONTEXT: '/api/context',
    ROUTINE_EVENTS: '/api/routine/events',
    ROUTINE_SUMMARY: '/api/routine/summary',
    ROUTINE_LATEST: '/api/routine/latest-event',
    ROUTINE_LATEST_PATH: '/api/routine/latest-event',
    ROUTINE_PROCESS_COMMAND: '/api/routine/process-command',
    HEALTH: '/api/health'
};
...
function updateSyncStatus() {
    ...
    const unsyncedFeedEvents = getLocalEvents('feed').filter(event => !event.synced).length;
...
function clearOldEvents() {
    try {
        ...
        ['sleep', 'sleep_end', 'feed'].forEach(eventType => {
            let storageKey;
...
async function sendMessage() {
    ...
    try {
        ...
        if (isRoutineCmd) {
            ...
            serverAvailable = await fetch('/api/health', { 
        ...
        const response = await fetch(API_ENDPOINTS.CHAT, {
            ...
            body: JSON.stringify({
                ...
                thread_id: threadId,
                language: language,
                local_event_id: localEventId // Pass the local event ID to the server
            }),
        ...
        if (isRoutineCmd) {
            ...
            // Don't fetch summary again for summary requests
            if (!isSummaryEvent) {
                ...
                // Try to sync events after a successful command
                setTimeout(() => {
                    syncLocalEvents();
                }, 2000);
            }
...
function generateLocalSummary(period = 'day') {
    try {
        ...
        const feedEvents = getLocalEvents('feed');
...
async function syncLocalEvents() {
    ...
    try {
        ...
        const feedEvents = getLocalEvents('feed').filter(event => !event.synced);
        ...
        for (const event of sleepEvents) {
            try {
                ...
                const timeMatch = event.message.match(/(\d{1,2}(?::\d{2})?\s*(?:am|pm|a\.m\.|p\.m\.)?|\d{1,2}(?::\d{2})?(?:\s*(?:בבוקר|בערב|בצהריים|בלילה))?)/i);
                ...
                if (timeMatch) {
                    ...
                    if (parsedTime) {
                        // Create a new date with the parsed time but keep the original date
                        const originalDate = new Date(event.start_time);
                        const parsedDate = new Date(parsedTime);
                        
                        // Create a new date with the original date and the parsed time
                        ...
                        eventTime = combinedDate.toISOString();
                        console.log(`Parsed time from message: ${timeMatch[1]} → ${new Date(eventTime).toLocaleTimeString()}`);
                    }
                }
            ...
            try {
                ...
                if (timeMatch) {
                    ...
                    if (parsedTime) {
                        ...
                        console.log(`Parsed time from message: ${timeMatch[1]} → ${new Date(endTime).toLocaleTimeString()}`);
                    }
                }
                
                // Get the latest sleep event for this thread from the server
                try {
                    const response = await fetch(`${API_ENDPOINTS.ROUTINE_LATEST_PATH}/${event.thread_id}/sleep`);
        ...
        for (const event of feedEvents) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Mark as synced in localStorage
                    markEventAsSynced('feed', event.local_id, data.event_id);
...
function parseTimeFromString(timeStr) {
    try {
        ...
        // Handle potential concatenated times (e.g., "07:0009:51")
        // This pattern looks for time formats that might be stuck together
        const concatenatedMatch = timeStr.match(/(\d{1,2}:\d{2})(\d{1,2}:\d{2})/);
        if (concatenatedMatch) {
            // Use the first time found
            timeStr = concatenatedMatch[1];
            console.log(`Fixed concatenated time: ${concatenatedMatch[0]} → ${timeStr}`);
        }
        ...
        const hebrewMorning = /בבוקר|לפנה״צ|לפנהצ/i.test(timeStr);
        const hebrewAfternoon = /אחה״צ|אחהצ|בצהריים/i.test(timeStr);
        const hebrewEvening = /בערב|בלילה/i.test(timeStr);
        
        // Handle "8:30" format
        if (timeStr.includes(':')) {
            const [hours, minutesPart] = timeStr.split(':');
            
            // Extract just the numbers from minutes part (in case it has AM/PM or Hebrew text)
            const minutesMatch = minutesPart.match(/^\d{1,2}/);
            const minutes = minutesMatch ? parseInt(minutesMatch[0]) : 0;
            
            let hour = parseInt(hours);
            
            // Adjust for AM/PM or Hebrew time indicators
            const isPM = timeStr.includes('pm') || hebrewAfternoon || hebrewEvening || 
                         /p\.m\./i.test(timeStr) || /אחה"צ/i.test(timeStr) || /אחר הצהריים/i.test(timeStr);
            const isAM = timeStr.includes('am') || hebrewMorning || 
                         /a\.m\./i.test(timeStr) || /לפנה"צ/i.test(timeStr) || /לפני הצהריים/i.test(timeStr);
            
            // Special handling for 12 hour
            if (hour === 12) {
                if (isAM) {
            ...
            // Special case for Hebrew morning which is explicit
            if (hebrewMorning && hour >= 12) {
                hour -= 12;
            }
            
            // Default assumption for hours 1-11 with no AM/PM indicator
            if (!isPM && !isAM && hour >= 1 && hour <= 11) {
                // Get the current hour to make a contextual guess
                const currentHour = new Date().getHours();
                
                // If the hour is 7-11, assume AM
                if (hour >= 7 && hour <= 11) {
                    // Keep as AM
                } 
                // If the hour is 1-6, make a contextual guess
                else if (hour >= 1 && hour <= 6) {
                    // If current time is evening/night (6pm-6am), assume PM for 1-6
            ...
            return today.setHours(hour, minutes);
        }
        
        // Handle simple hour format (just a number)
        ...
        if (hourMatch) {
            ...
            // Special case for Hebrew morning which is explicit
            if (hebrewMorning && adjustedHour >= 12) {
                adjustedHour -= 12;
            }
        ...
        console.error('Error parsing time:', error, 'for string:', timeStr);
        return null;
    }
}
